...
BUZZER = 12#bcm pin number

GPIO.setmode(GPIO.BCM)#set mode to bcm
GPIO.setup(BUZZER,GPIO.OUT)#setup buzzer as output
...
treshold = 0.2#set threshold for movement detection
#initial values
prev_ax, prev_ay, prev_az = mpu.get_acceleration()#store initial values
while True:
    ax, ay, az = mpu.get_acceleration()#get acceleration data
    gx, gy, gz = mpu.get_gyroscope()#get gyroscope data
    temp = mpu.get_temperature()#get temperature data
    movement = abs(ax - prev_ax) + abs(ay - prev_ay) + abs(az - prev_az)#calculate the total movement for each axis and add them together
    ...
    print(f"Acceleration {ax} x {ay} y {az} z")#print acceleration
    print(f"gyroscope {gx} x {gy} y {gz} z")#print gyroscope
    print(f"movement {movement}")#print movement
    prev_ax, prev_ay, prev_az = ax, ay, az#update previous values
