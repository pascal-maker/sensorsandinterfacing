...
def bcd_changed(channel):#when a button is pressed this function is called
    ...
    btn_num = pins.index(channel) + 1  # which button (1-4) triggered the event
    ...
    for i, pin in enumerate(pins):#loops through the pins gives you both the index i and pin number at the same time
        bit = 1 - GPIO.input(pin)   #returns 1 if not pressed and 0 if pressed subteatcin  from 1 flips it
        print(f"  Button {i+1} (GPIO {pin}): {'PRESSED' if bit else 'released'} -> bit {i} -> contributes {bit * (2**i)}")
        value |= (bit << i)# updates the value of the bcd code based on the bits read from the buttons shifts the bit elft into is correct position  the OR assignment operater |=  adds the bit to the value

    bcd_value = value#updates the global variable with the new value rest of program can use it
    print(f"  => BCD value: {bcd_value} (binary: {bcd_value:04b})")
...
try:
    while True:
        if bcd_value == 0:#if the bcd value is 0 then turn on the LED for 1 second and then turn it off IF ALL BUTTONS ARE UNPRESSED VALUE = 0 LED TURNS ON AND STAYS ON FOR 1 SECOND. THEN THE LOOP REPEATS AND KEEPS IT ON AGAIN SO EFFIECLERY THE LED JUST STAYS ON WHILE NOTHING IS PRESSED.
            ...
            interval = 1 / (bcd_value * 2)#sets the interval between flashes based on the bcd value CALCUATE TE TINE BETWEEN EACH TOGGLE FOR EXAMPLE IF bcd value = 1 it will flash 2 times the interval will be 1/(1*2) = 0.5 seconds so it will turn on for 0.5 seconds and then off for 0.5 seconds and repeat until the total time for this bcd value has passed so if interval is 0.5 and it flashes 2 times total time = 0.5*2 =1 second THE HIGHER THE NUMBER , THE FASTER IT BLINKS

            for _ in range(bcd_value * 2):#the number of flashes is equal to the bcd value loops   BECUASE ONE FULL BLINK 2 TOGGLES ON AND OFF
                GPIO.output(led, not GPIO.input(led))#flips the state of the LED READS THE CURRENT PIN STATE AND NOT FLIPS IT THEN WRITES IT BACK SO EACH ITERAATON TOGGLWS THE LED. THEN WIATS INTERVAL SECONDS BEFORE THE NEXT TOGGLE
